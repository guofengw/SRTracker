...
#include <cv.h>
#include <cxcore.h>
/*********************** by wgf at 2012/12/01*******************************/

template<typename T>
inline void LCC_Coding(Matrix<T>& y, Matrix<T>& D, SpMatrix<T>& a, int knn = 10, float beta = 0.01)
{
	//先计算K-NN
	//求2向量之间的2范数
	Vector<T> vy;
	...
	if(knn > D.n())   //防止knn个数大于模板个数
		knn = D.n();
	Sort temp;    //临时变量
	for(int i=0; i<knn; i++)   //取knn个数就可以了
	{
		for(int j=i+1; j<D.n(); j++)
		{
			if(ps[i].data > ps[j].data)
			{
				temp.data = ps[i].data;
				temp.num = ps[i].num;
				ps[i].data = ps[j].data;
	...
	Matrix<T> DtD(D.m(),knn);
	for(int i=0; i<knn; i++)
	{
		D.getData(vd,ps[i].num);
		for(int j=0; j<D.m(); j++)
		{
			DtD(j,i) = vd[j] - vy[j];
		}
	...
	Dt.mult(DtD,CC,false,false,1.0,0.0);
	II.eye();
	CC.add(II,CC.trace()*beta);
	

	CvMat *cvCC = cvCreateMat(CC.m(),CC.n(),CV_64FC1);
	for(int i=0;i<CC.n(); i++)
	{
		for(int j=0; j<CC.m(); j++)
		{
			cvCC->data.db[i*CC.m()+j] = CC(j,i);
		}
	}


	CvMat *cvOne = cvCreateMat(knn,1,CV_64FC1);
	for(int i=0;i<knn; i++)
		cvOne->data.db[i] = 1;
	CvMat *cvCoeff = cvCreateMat(knn,1,CV_64FC1);
	cvSolve(cvCC,cvOne,cvCoeff);    //solve the equation: Ax=1
	double sum=0;
	for(int i=0;i<knn; i++)         //sum of coeff = 1
	{
		sum += cvCoeff->data.db[i];
	}
	for(int i=0; i<knn; i++)
	{
		cvCoeff->data.db[i] /= sum;
	}
